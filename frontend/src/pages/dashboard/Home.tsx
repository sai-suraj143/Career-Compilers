  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setTrips(data);
        const meta = await fetchAnalytics();
        setAnalytics(meta);
      } catch (error: unknown) {
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        <Card className="space-y-4">
          ...
          <p className="text-sm text-slate-400">Keep planning. Your next journey is waiting.</p>
        </Card>
        <Card className="space-y-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-sm uppercase tracking-[0.3em] text-slate-500">Budget</p>
              <p className="mt-3 text-3xl font-semibold text-white">{formatCurrency(budgetSummary)}</p>
            </div>
            <div className="rounded-3xl bg-indigo-500/10 p-3 text-indigo-300">
              <Wallet className="h-5 w-5" />
            </div>
          </div>
          <p className="text-sm text-slate-400">A quick pulse of your planning overhead.</p>
        </Card>
        <Card className="space-y-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-sm uppercase tracking-[0.3em] text-slate-500">Highlights</p>
              <p className="mt-3 text-3xl font-semibold text-white">{analytics?.popularCities?.length ?? 0}</p>
            </div>
            <div className="rounded-3xl bg-fuchsia-500/10 p-3 text-fuchsia-300">
              <Sparkles className="h-5 w-5" />
            </div>
          </div>
          <p className="text-sm text-slate-400">Top travel destinations recommended for you.</p>
        </Card>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.4fr_0.8fr]">
        <Card className="space-y-6">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <h3 className="mt-3 text-xl font-semibold text-white">Plan or continue your journey</h3>
            </div>
          ...
          {loading ? (
            <div className="grid gap-4">
              {Array.from({ length: 3 }).map((_, idx) => (
                <div key={idx} className="h-28 rounded-3xl bg-slate-800/60" />
              ))}
            </div>
          ) : trips.length === 0 ? (
            <div className="rounded-3xl border border-dashed border-slate-700 p-8 text-center text-slate-400">No trips found. Create your first trip to get started.</div>
          ) : (
            <div className="grid gap-4">
              {trips.slice(0, 3).map((trip) => (
                <div key={trip.id} className="rounded-3xl border border-white/10 bg-slate-950/80 p-5 transition hover:border-violet-400/30 hover:bg-slate-900/90">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                    ...
                    <span className="rounded-full bg-white/5 px-3 py-1 text-xs uppercase tracking-[0.3em] text-slate-300">{trip.visibility ?? 'PRIVATE'}</span>
                  </div>
                  <p className="mt-3 text-sm leading-6 text-slate-400">{trip.description || 'No description added yet.'}</p>
                </div>
              ))}
            </div>
          )}
        ...
        <Card className="space-y-6">
          <div>
            ...
            <h3 className="mt-3 text-xl font-semibold text-white">Popular destinations</h3>
          </div>
          <div className="space-y-4">
            {(analytics?.popularCities || ['Paris', 'Tokyo', 'Bali']).map((city) => (
              <div key={city} className="flex items-center justify-between rounded-3xl border border-white/5 bg-slate-950/70 px-4 py-4">
                <div>
                  <p className="font-semibold text-white">{city}</p>
                  <p className="text-sm text-slate-400">Trending city for curated experiences</p>
                </div>
